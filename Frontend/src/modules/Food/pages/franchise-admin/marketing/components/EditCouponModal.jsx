...
import { 
  ...
  Store, Pizza, FolderOpen, ChevronRight, Save 
} from "lucide-react";
...
export default function EditCouponModal({ visible, onClose, onSubmit, coupon }) {
  ...
  // Prefill when coupon prop loads/changes
  useEffect(() => {
    if (coupon) {
      reset({
        ...
        usagePerCustomer: coupon.usagePerCustomer || null,
  ...
  const handleDiscountTypeChange = (type) => {
    ...
    if (type === "free-delivery") {
      ...
      setValue("maximumDiscount", null, { shouldDirty: true });
    }
  };
  ...
  const onFormSubmit = (data) => {
    ...
    onSubmit(coupon._id, normalizedData);
  };
        ...
        <aside className="w-[240px] bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 flex flex-col justify-between shrink-0">
          <div>
            <div className="p-4 border-b border-zinc-150 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30">
              <span className="p-1.5 bg-[var(--primary)] text-white rounded-lg inline-block mb-1.5 font-bold">
          ...
          <div className="p-4 border-t border-zinc-150 dark:border-zinc-800 text-[9px] text-zinc-400 font-bold leading-normal">
            * Dirty state detection enabled. Save is enabled only on edits.
          </div>
        </aside>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-zinc-950">
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
            ...
            {activeTab === "applicability" && (
              <div className="space-y-4 max-w-2xl animate-fade-in font-semibold text-zinc-700 dark:text-zinc-350">
                <div className="space-y-1">
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        value={field.value}
                        onChange={(val) => field.onChange(val, { shouldDirty: true })}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <button
                      key={target.id}
                      type="button"
                      onClick={() => setValue("customerType", target.id, { shouldDirty: true })}
                      className={`p-3.5 border rounded-xl flex flex-col text-left justify-between cursor-pointer transition-all duration-150 ${
          ...
          <footer className="p-4 border-t border-zinc-150 dark:border-zinc-800 flex justify-end gap-2 bg-zinc-50/30 dark:bg-zinc-900/10 shrink-0">
            ...
            <button
              type="submit"
              disabled={!isDirty}
              className={`px-5 py-2 text-white font-black rounded-xl shadow-md cursor-pointer transition-all active:scale-[0.98] inline-flex items-center gap-1 ${
                isDirty 
                  ? "bg-[var(--primary)] hover:bg-[var(--primary-hover)] opacity-100 cursor-pointer" 
                  : "bg-zinc-350 dark:bg-zinc-800 opacity-60 cursor-not-allowed"
              }`}
            >
              <Save size={14} />
