...
import { z } from "zod";
import { X, Sparkles, Upload, Package, ClipboardCheck, DollarSign, AlertTriangle } from "lucide-react";
import { toast } from "sonner";
import { useIngredientDetails, useSuppliers } from "../hooks/useIngredients";
...
export default function EditIngredientModal({ isOpen, onClose, onSubmit, ingredientId }) {
  ...
  // Reset form when active ingredient changes
  useEffect(() => {
    if (isOpen && ingredient) {
      reset({
        ...
        idealStock: ingredient.idealStock || 0,
        costPerUnit: ingredient.costPerUnit || 0,
        shelfLife: ingredient.shelfLife || 0,
        expiryTracking: ingredient.expiryTracking || false,
        supplierId: ingredient.supplierId || "",
        isDefaultSupplier: ingredient.isDefaultSupplier || false
      });
      setActiveTab("basic");
    }
  ...
  // Handle safe closing with dirty state detection
  ...
  const handleMediaUpload = (e) => {
    ...
    setTimeout(() => {
      ...
      setValue("image", randomImage, { shouldDirty: true });
  ...
  return (
    <div className="fixed inset-y-0 right-0 left-0 lg:left-[280px] z-50 overflow-hidden text-xs font-semibold text-zinc-700 dark:text-zinc-350">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/45 backdrop-blur-xs z-40 animate-fade-in" onClick={handleSafeClose} />
        ...
        <div className="w-full max-w-[850px] bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-scale-up">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-extrabold text-sm text-zinc-900 dark:text-white">
                  Edit Raw Material: {ingredient?.name || "Loading..."}
                </h3>
                {ingredient && (
                  <p className="text-[9px] text-zinc-400 font-bold mt-0.5">
                    Last Updated: {new Date(ingredient.updatedAt).toLocaleString("en-IN")} by {ingredient.createdBy || "Admin"}
                  </p>
                )}
              </div>
            </div>
            <button onClick={handleSafeClose} className="p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-900 text-zinc-400">
              <X size={15} />
            </button>
          </header>
          ...
          {detailsLoading ? (
            <div className="h-96 flex flex-col items-center justify-center space-y-3">
              ...
              <p className="font-bold text-zinc-450">Retrieving details...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit(onFormSubmit)} className="flex-1 overflow-hidden flex flex-col bg-white dark:bg-zinc-950">
                ...
                {activeTab === "basic" && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <label className="block text-zinc-500 font-bold">Ingredient Code *</label>
                        <input
                          type="text"
                      ...
                      <div className="p-4 border-2 border-dashed border-zinc-250 dark:border-zinc-800 rounded-2xl flex items-center gap-4 bg-zinc-50/30 dark:bg-zinc-900/10">
                        ...
                        <div className="flex-1">
                          <input
                            type="file"
                            accept="image/*"
                            id="edit-ingredient-file-input"
                            onChange={handleMediaUpload}
                            className="hidden"
                          />
                          <label
                            htmlFor="edit-ingredient-file-input"
                          ...
                          >
                            <Upload size={11} />
                            <span>Replace Raw Material Banner</span>
                          </label>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center h-10 border border-zinc-150 dark:border-zinc-855 rounded-xl bg-zinc-50 dark:bg-zinc-900 px-3 justify-between">
              ...
              {/* Footer Buttons */}
              <footer className="p-4 border-t border-zinc-150 dark:border-zinc-855 flex justify-between bg-zinc-50/30 dark:bg-zinc-900/10 shrink-0">
                <div>
                  {activeTab === "inventory" && (
                    <button
                      type="button"
