...
import { TrendingUp, ShoppingBag, Clock, Star, Target, X, CheckCircle2, Award, Zap } from "lucide-react"
import {
  ResponsiveContainer,
  AreaChart,
  Area,
...
import { getDeliveryPartnerPerformance } from "../mockManagersData"

export default function RiderPerformanceModal({ isOpen, onClose, rider }) {
  const [timeframe, setTimeframe] = useState("daily")

  if (!isOpen || !rider) return null

  const data = getDeliveryPartnerPerformance(rider.id)
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl overflow-hidden animate-scale-up z-10">
        ...
        <div className="flex items-center justify-between p-4 border-b border-zinc-150 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/20">
          <div className="flex items-center gap-2 text-zinc-900 dark:text-white">
            ...
            <h3 className="text-sm font-extrabold uppercase tracking-wider">Rider Performance Analytics</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto scrollbar-thin">
          <div className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-950/20 border border-zinc-100 dark:border-zinc-850 rounded-xl">
            ...
            />
            <div>
              <p className="text-[11px] font-black text-zinc-800 dark:text-zinc-250 leading-none">{rider.name}</p>
              <p className="text-[9px] text-zinc-500 font-semibold mt-1">ID: {rider.employeeCode} • Vehicle: {rider.vehicleType}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
            {[
              { label: "Total Deliveries", val: data.totalDeliveries, sub: "MTD Deliveries", icon: ShoppingBag, color: "text-blue-500 bg-blue-50 dark:bg-blue-950/20" },
              { label: "Avg Speed", val: `${data.avgDeliveryTime}m`, sub: "Order ETA speed", icon: Clock, color: "text-amber-500 bg-amber-50 dark:bg-amber-950/20" },
              { label: "Avg Rating", val: `${data.avgRating}★`, sub: "Feedback score", icon: Star, color: "text-yellow-600 bg-yellow-50 dark:bg-yellow-950/20" },
              { label: "Distance Covered", val: `${data.distanceCovered}km`, sub: "Total distance", icon: Zap, color: "text-emerald-500 bg-emerald-50 dark:bg-emerald-950/20" },
              { label: "Cancellation Rate", val: `${data.cancellationRate}%`, sub: "Dispatched errors", icon: X, color: "text-rose-500 bg-rose-50 dark:bg-rose-950/20" }
            ].map((stat, idx) => (
              <div key={idx} className="bg-zinc-50 dark:bg-zinc-950/25 border border-zinc-100 dark:border-zinc-850 rounded-xl p-2 text-center flex flex-col justify-between min-h-[75px]">
                <div>
                  <span className="block text-[8px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-wider truncate">{stat.label}</span>
                  <span className="text-xs font-black text-zinc-800 dark:text-white mt-0.5 block">{stat.val}</span>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="border border-zinc-150 dark:border-zinc-855 bg-white dark:bg-zinc-900 rounded-2xl p-4 flex flex-col justify-between">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Deliveries Output Trend</span>
              ...
              <div className="h-[140px] w-full text-xs">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={activeChartData} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
                    <defs>
                      <linearGradient id="colorRiderDeliveries" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <Area type="monotone" dataKey="deliveries" stroke="var(--primary)" fillOpacity={1} fill="url(#colorRiderDeliveries)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="border border-zinc-150 dark:border-zinc-855 bg-white dark:bg-zinc-900 rounded-2xl p-4 flex flex-col justify-between">
              <div className="mb-3">
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Weekly Rating Index</span>
              </div>
              ...
              <div className="h-[140px] w-full text-xs">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.trends.rating} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ fontSize: 9 }} />
                    <Line type="monotone" dataKey="rating" stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
