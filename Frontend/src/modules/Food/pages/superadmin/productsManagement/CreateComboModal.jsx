...
import { X, CloudUpload, ArrowRight, ArrowLeft, Trash2, CheckCircle2, Search, Sparkles, Check, CheckSquare, Square, Tag, Trash } from "lucide-react";

export default function CreateComboModal({
  isOpen,
  onClose,
  products = [],
  franchises = [],
  ...
  const [image, setImage] = useState("https://images.unsplash.com/photo-1513104890138-7c749659a591?fm=webp&fit=crop&w=300&q=80");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setDescription("");
      setComboType("fixed");
      setImage("https://images.unsplash.com/photo-1513104890138-7c749659a591?fm=webp&fit=crop&w=300&q=80");
      setSelectedItems([]);
      setProdSearch("");
      setProdCategory("");
  ...
  const addProductToCombo = (prodId) => {
    setSelectedItems(prev => {
      ...
      if (exists) {
        return prev; // Prevent duplicates, let quantity selector handle it
      }
  ...
  const handleSaveCombo = (isDraft = false) => {
    // Validate everything
    ...
    alert(`Successfully ${isDraft ? "saved draft of" : "published"} "${name}"!`);
  };
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[70] transition-opacity duration-300" onClick={onClose} />
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-xl bg-white dark:bg-zinc-950 shadow-2xl z-[80] flex flex-col transform transition-transform duration-300 animate-in slide-in-from-right-5 text-zinc-900 dark:text-zinc-100 select-none">
        ...
        <div className="px-4 h-14 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-sm text-black dark:text-white">Create Promotional Combo Offer</h3>
            <p className="text-[10px] text-zinc-400 font-bold mt-1 uppercase tracking-wider">
              Step {currentStep} of 6 — {
                currentStep === 1 ? "Basic Information" :
                currentStep === 2 ? "Product Selection" :
                currentStep === 3 ? "Pricing & Margin calculations" :
                currentStep === 4 ? "Validity Period" :
                currentStep === 5 ? "Availability Scope" :
                "Publish Profile"
              }
            </p>
          </div>
          <button onClick={onClose} className="p-1 hover:bg-zinc-150 dark:hover:bg-zinc-850 rounded-full text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 cursor-pointer">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
          ...
          {currentStep === 1 && (
            <div className="space-y-3.5 animate-fadeIn">
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  rows={3}
                  placeholder="Summarize the menu bundle for customers (e.g. 1 Personal Veg Pizza, Garlic Bread and Beverage)."
                  value={description}
              ...
              {/* Drag Drop Mock Area */}
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest font-bold">Image Attachment (WebP Preview)</label>
                <div className="border-2 border-dashed border-zinc-200 dark:border-zinc-800 p-6 rounded-xl flex flex-col items-center justify-center gap-2 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                  ...
                  <input type="file" onChange={handleImageUpload} accept="image/webp" className="hidden" id="banner-uploader" />
                  <label htmlFor="banner-uploader" className="px-3 py-1 bg-zinc-150 dark:bg-zinc-800 text-[10px] rounded font-bold cursor-pointer border border-zinc-250 dark:border-zinc-750">Choose File</label>
                  {image && (
          ...
          {currentStep === 2 && (
            <div className="space-y-3.5 animate-fadeIn">
              ...
              {/* Selector List Area */}
              <div className="border border-zinc-150 dark:border-zinc-800 rounded-xl max-h-[160px] overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-850 p-1 bg-zinc-50/50 dark:bg-zinc-900/35 scrollbar-thin">
              ...
              <div className="space-y-1.5">
                <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Selected Components *</h4>
                
                {selectedItems.length === 0 ? (
                  <p className="text-xs text-zinc-400 italic py-6 text-center border border-dashed rounded-xl">No items selected yet. Add items using the selection box above.</p>
                ) : (
          ...
          {currentStep === 3 && (
            <div className="space-y-3.5 animate-fadeIn">
              ...
              <div className="grid grid-cols-2 gap-3 bg-zinc-50 dark:bg-zinc-900/50 p-3.5 border border-zinc-150 dark:border-zinc-800 rounded-xl shadow-inner text-xs font-semibold">
                <div>
                  <span className="text-[9px] text-zinc-400 font-bold uppercase block mb-0.5">Sum of Items (A-la-carte)</span>
                  <p className="text-sm font-black font-mono text-zinc-700 dark:text-zinc-300">₹{originalTotal.toFixed(2)}</p>
                </div>
                <div>
                  <span className="text-[9px] text-emerald-500 font-bold uppercase block mb-0.5">Customer Savings</span>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="px-2.5 py-1.5 border border-zinc-200 dark:border-zinc-800 rounded-lg text-xs bg-zinc-50 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-350 focus:outline-none focus:ring-1"
                  />
                </div>
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className={`px-2.5 py-1.5 border rounded-lg text-xs bg-zinc-50 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-350 focus:outline-none focus:ring-1 ${errors.validTo ? 'border-red-500' : 'border-zinc-200 dark:border-zinc-800'}`}
                  />
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="px-2.5 py-1.5 border border-zinc-200 dark:border-zinc-800 rounded-lg text-xs bg-zinc-50 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 focus:outline-none"
                  />
                </div>
                <div className="flex flex-col gap-1">
          ...
          {/* STEP 5: Store & Franchise applicability */}
          {currentStep === 5 && (
            <div className="space-y-3.5 animate-fadeIn">
              
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Scope Applicability *</label>
                <div className="flex flex-col sm:flex-row gap-4 p-3 border border-zinc-150 dark:border-zinc-800 rounded-xl bg-zinc-55/30 dark:bg-zinc-900/20 text-xs">
          ...
          {currentStep === 6 && (
            <div className="space-y-3.5 animate-fadeIn">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-bold text-zinc-750">
                  {[
              ...
              <div className="p-4 border border-emerald-200 dark:border-emerald-950/20 bg-emerald-500/5 rounded-xl flex items-start gap-3 text-xs text-emerald-800 dark:text-emerald-400 leading-relaxed font-semibold">
                <CheckCircle2 className="text-emerald-500 shrink-0 mt-0.5 animate-pulse" size={16} />
                <div>
                  <h4 className="font-bold text-emerald-700 dark:text-emerald-400">Combo validation successful</h4>
                  <p className="text-[10px] text-emerald-600/80 dark:text-emerald-500/80 mt-0.5 leading-snug">All mandatory attributes, items list, store coverage permissions, and discount margins have been verified and comply with brand rules.</p>
                </div>
              </div>
        ...
        <div className="p-3 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => handleSaveCombo(true)}
              className="px-3 py-1.5 border border-zinc-200 dark:border-zinc-800 hover:border-[var(--primary)] text-zinc-650 hover:text-[var(--primary)] dark:text-zinc-350 rounded-lg text-xs font-bold hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors cursor-pointer"
            >
              Save Draft
            </button>
