...
import { 
  ...
  Mail, MessageSquare, Layers, Loader2 
} from 'lucide-react';
import { 
  apiGetCoupons, 
  apiCreateCampaign 
...
export default function CreateCampaign({ isOpen, onClose, onCampaignCreated }) {
  ...
  const validateStep = (step) => {
    ...
    if (step === 2) {
      // Regions or segment checks if required, normally optional
    }
    if (step === 3) {
  ...
  const handleSave = async () => {
    ...
    try {
      const newCamp = await apiCreateCampaign(campaignPayload);
      setLoading(false);
      onCampaignCreated && onCampaignCreated(newCamp);
      handleClose();
    } catch (err) {
      setLoading(false);
  ...
  const handleClose = () => {
    ...
    setCustomerSegment('All Customers');
    setRegionIds([]);
    setFranchiseIds([]);
    setStoreIds([]);
    setCouponId('');
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-[1200px] h-[700px] max-h-[90vh] rounded-2xl flex flex-col shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden animate-in fade-in zoom-in-95 duration-250">
        ...
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900/50">
          <div>
            ...
            <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5">Design a new targeted campaign and promotions</p>
          </div>
          <button 
            onClick={handleClose}
          ...
          <div className="w-64 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/30 p-4 flex flex-col justify-between">
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 flex gap-2">
              ...
              <p className="text-[10px] text-amber-800 dark:text-amber-300 leading-relaxed font-medium">
                Ensure campaign coupons are active to drive high conversion rates.
              </p>
            </div>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea 
                      ...
                      placeholder="Write brief description summarizing the promo objective, discounts offered, and main items included..."
                      rows={4}
            ...
            {currentStep === 3 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-200">
                ...
                <div className="space-y-2 relative">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      placeholder="Type coupon code or title to search..."
                      ...
                      {couponsLoading ? (
                        ...
                        searchedCoupons.map(cpn => (
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 hover:bg-zinc-50 dark:hover:bg-zinc-850/50 flex justify-between items-center transition-colors"
                          >
                            <div>
                ...
                {selectedCoupon ? (
                  ...
                  <div className="border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl p-8 text-center bg-zinc-50/50 dark:bg-zinc-950/50">
                    <Gift size={24} className="mx-auto text-zinc-400 dark:text-zinc-650" />
                    <p className="text-xs font-bold text-zinc-600 dark:text-zinc-400 mt-2">No Coupon Mapped Yet</p>
                    <p className="text-[10px] text-zinc-500 dark:text-zinc-500 mt-0.5">Search and select an active coupon code from the box above to link it.</p>
                  </div>
                )}
              </div>
            )}
