...
export default function PauseAssemblyModal({ visible, onClose, item, onPause }) {
  ...
  const handleSubmit = () => {
    ...
    setReason("Ingredient unavailable");
    setNotes("");
    onClose();
  };
    ...
    <Modal
      title={
        <div className="flex items-center gap-2 border-b border-slate-100 dark:border-zinc-800 pb-3">
          ...
          <div>
            <h3 className="text-base font-black text-slate-900 dark:text-white">Pause Pizza Assembly</h3>
            <p className="text-[10px] font-bold text-slate-400 dark:text-zinc-550 mt-0.5">
              Halt toppings assembly timer for {item.name}
            </p>
          </div>
        </div>
      }
    ...
    >
      <div className="py-4 space-y-3.5 text-xs">
        <div className="bg-amber-50 dark:bg-amber-950/20 text-amber-800 dark:text-amber-400 p-2.5 rounded-xl text-[10px] font-bold border border-amber-100 dark:border-amber-900/30 flex gap-1.5 items-start">
          ...
          <span>Pausing freezes the stopwatch and flags this item on the dashboard.</span>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            options={[
              { value: "Ingredient unavailable", label: "Ingredient unavailable (e.g. Cheese shortage)" },
              { value: "Equipment issue", label: "Equipment issue (workstation block)" },
              { value: "Chef unavailable", label: "Chef reassigned" },
              { value: "Quality check", label: "Quality check failed" },
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            placeholder="Describe specific halt details..."
            rows={3}
